...
//                .replace(/\$\{([^\}]*)\}/g, '{{= $1}}')
                .replace(/\{\{(\/?)(\w+|.)(?:\(((?:[^\}]|\}(?!\}))*?)?\))?(?:\s+(.*?)?)?(\(((?:[^\}]|\}(?!\}))*?)\))?\s*\}\}|\$\{([^\}]*)\}/g,
                    function(all, slash, type, fnargs, target, parens, args, exval) {
                        //literal tag
                        var callee = arguments.callee;
                        if(type=='literal') {
                            if(!slash) {
                                callee.$literaltag = target;
                                callee.$literal = true;
                            } else if(callee.$literal && callee.$literaltag == target) {
                                delete callee.$literal;
                                delete callee.$literaltag;
                            } else {
                                return all;
                            }
                            return '';
                        ...
                        if(exval) {
                            return callee(all, false, '=', undefined, exval);
                        }
